...
int main()
{
    // initalize varibles so we can started processing commands
	char prompt = '#';	
	string commandLine;
	...
	while(!error)
		{
            ...
            // create copy of string into a char and account for terminating character
			...
			if (error)
				cout << endl << stderr << "Invalid command" << endl;	
		}
...
bool replayCommand(char *arguments[], vector<string> &history)
{
	...
	if(!(strcmp(command, "history")))
		...
		terminateAllCommand(replayCommand);
