...
public class Pathogen
{
	...
	public static HashSet<String> findPaths(char [][] maze)
	{
		...
		Queue<Maybe> queue = new LinkedList<Maybe>();
		...
		return findPaths(maze, visitedStates, height, width, newPath, newSet, queue(startCol, startRow));
	}


	private static HashSet<String> findPaths(char [][] maze, char [][] visited,
	                                 int height, int width, StringBuilder path, HashSet<String> set, Queue<Maybe> queue)
	{
		// NO CHANGING MAZE ARRAY
		...
		while(!queue.isEmpty())
		{
			Maybe current = queue.remove();
			if (visited[current.row][current.col] == 'e')
			{
				// deletes last space in String
				path.setLength(path.length() - 1);
				// System.out.println("String: " + path.toString());
		...
		if(visited[current.row][cuurent.col] != BREADCRUMB)

			for (int i = 0; i < possibleMoves.length; i++)
			{
				// change state
				int newRow = current.row + possibleMoves[i][0];
				int newCol = current.col + possibleMoves[i][1];
		...
		// if no solution exists returns empty HashSet
	...
	public static void main(String [] args) throws IOException
	{
		...
		HashSet<String> answer = Pathogen.findPaths(maze);
