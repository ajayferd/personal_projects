...
/* DATAPATH Signals */// names of instruction sections
//
//
...
unsigned op,//instruction [31-26]
        ...
        // Register output
        unsigned data1,data2;// sign extend
        unsigned extended_value;// ALU result
        unsigned ALUresult;
char Zero;// data read from Memory
unsigned memdata;
/******/
...
void Init(void){memset(Reg, 0, (REGSIZE + 4) * sizeof(unsigned));
NREG("pc") = PCINIT;
NREG("sp") = SPINIT;
NREG("gp") = GPINIT;
}
void DisplayControlSignals(void){
    fprintf(stdout, "\tControl Signals: %0x%0x%0x%0x%03x%0x%0x%0x%0x\n", controls.RegDst,
            controls.Jump, controls.Branch, controls.MemRead, controls.MemtoReg, controls.ALUOp,
...
void Step(void){
    ...
    {
        /* write to register */
        write_register(r2,r3,memdata,ALUresult,controls.RegWrite,controls.RegDst,
                       controls.MemtoReg,Reg);
...
void DumpReg(void){
    ...
    for (i = 0; i < REGSIZE + 4; i++){
        fprintf(stdout, "%s %s%s %08x%s",(i % 4 == 0)
        ? Redir : "",RegName[i], bb + strlen(RegName[i]) * sizeof(char),Reg[i], (i % 4 == 3) ? "\n" : "     ");
    }
}
                     ...
                     case 's': case 'S':
                         ...
                         case 'h': case 'H':
                             ...
                             case 'p': case 'P':
                                 rewind(FP);sc = 0;
                                 while (!feof(FP)){
                                     if (fgets(Buf, BUFSIZE, FP))fprintf(stdout, "%s % 5d  %s", Redir, sc++, Buf);
                                 }
                                 break;
...
int main(int argc, char **argv){
    ...
    if (argc == 3){if (strcmp(argv[2], "-r") == 0){
        ...
        for (i = PCINIT; !feof(FP); i += 4){
            ...
            if (sscanf(Buf, "%lx", &t) != 1){
                fprintf(stderr, "%s: file %s error in line %d, continue...\n",argv[0],
                        argv[1], i - PCINIT + 1);MEM(i) = 0;
            }
            else{MEM(i) = strtoul(Buf, (char **) NULL, 16);
        }
   }
        Loop();
        fclose(FP);
